...
public abstract class MinecraftClientMixin {
    @Inject(method = "startIntegratedServer", at = @At(value = "NEW", target = "net/minecraft/world/level/LevelProperties"))
    ...
    private IntegratedServer createIntegratedServer(IntegratedServer original, @Share("newWorld") LocalBooleanRef newWorld) {
        ((MinecraftServerExtended) original).setspawnmod$getSeedHolder().setNewWorld(newWorld.get());
        return original;
    }
}
